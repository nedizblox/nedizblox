...
DescriptorSetLayout::Builder& DescriptorSetLayout::Builder::addBinding(
    ...
    assert(m_bindings.count(binding) == 0 && "Binding alerady in use");
...
DescriptorSetLayout::DescriptorSetLayout(
    ...
    std::vector<VkDescriptorBindingFlags> bindingFlags(setLayoutBindings.size(), 0);
...
void DescriptorPool::freeDescriptors(std::vector<VkDescriptorSet>& descriptors) const {
    vkFreeDescriptorSets(
        m_device.getDevice(), m_descriptorPool, static_cast<uint32_t>(descriptors.size()),
        descriptors.data());
}
...
void DescriptorWriter::overwrite(VkDescriptorSet& set) {
    ...
    vkUpdateDescriptorSets(
        m_pool.m_device.getDevice(), static_cast<uint32_t>(m_writes.size()), m_writes.data(), 0, nullptr);
}
