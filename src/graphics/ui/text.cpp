...
void Text::loadFont(const std::string& fontPath, vk::Sampler& sampler, mngrs::BindlessManager& bindlessManager) {
    ...
    for (uint8_t c = 0; c < 128; c++) {
        ...
        uint32_t glyphWidth = bitmap.width + (padding * 2);
        uint32_t glyphRows = bitmap.rows + (padding * 2);
        ...
        glm::vec2 uvBottomRight
            = {static_cast<float>(currentX + glyphWidth) / static_cast<float>(atlasWidth),
               static_cast<float>(currentY + glyphRows) / static_cast<float>(atlasHeight)};

        Character character
            = {uvTopLeft, uvBottomRight, glm::ivec2(glyphWidth, glyphRows),
               glm::ivec2(face->glyph->bitmap_left - padding, face->glyph->bitmap_top + padding),
               static_cast<uint32_t>(face->glyph->advance.x)};

        m_characters[c] = character;

        currentX += glyphWidth + 1;
        rowHeight = glm::max(rowHeight, glyphRows + 1);
    }
